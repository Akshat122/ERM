...
class LoginForm extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            $success = "<script> iziToast.show({title: 'Error', message: 'Maximum number of login attempts exceeded... please try after some time', theme: 'light',color: 'red',          position: 'topRight', progressBarColor: 'red', });   </script>";
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
                ...
                $success = "<script> iziToast.show({title: 'Error', message: 'Your account has been created, but you cannot log in until it has been verified', theme: 'light',color: 'red',          position: 'topRight', progressBarColor: 'red', });   </script>";

            } else {

                //Wrong username or password
                $success = "<script> iziToast.show({title: 'Error', message: 'Wrong username or password...', theme: 'light',color: 'red',          position: 'topRight', progressBarColor: 'red', });   </script>";
